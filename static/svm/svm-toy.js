...
function testAllPoints(model)
{
	...
	if(svmtype == 3 || svmtype == 4)
	{
		...
		var p = get_svr_epsilon(model) * height;

		for(var i = 0; i < width; i += 1)
		{
			var answer = libsvm_predict_for_toy(1.0*i/width, 0, model) * height;
	...
	for(var j = 0; j < height; j += 1)
	{
		for(var i = 0; i < width; i += 1, k += 1)
		{
			...
			// for one-class SVM, we replace -1 by +2
			if (svmtype == 2 && answer < 0) answer = 2;
